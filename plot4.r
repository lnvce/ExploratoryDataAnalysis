plot4<- function(my_working_dir="C:/Users/ealonvic/Documents/Rhmwk/exdata%2Fdata%2Fhousehold_power_consumption", DEBUGR=FALSE)
{
# Programiing assignment #4 of 4
# Requirements:
#
# 1) dplyr library needed to filter data clean up.
# 2) A data file, "household_power_consumption.txt" must be downloaded and saved to a local file system,
# 3) The function call supports two input parameters:
...
#
# Generate four plots and save as PNG / image.
# 
png(filename = "plot4.png",width = 480, height = 480)
# define a canvas for four plots with margins
par(mfrow=c(2,2), mar = c(4, 4, 4, 3))
#
# plot 1 of 4
with (sample_df, plot(Timestamp, Global_active_power, ylab="Global Active Power",xlab="", type="l") )
# plot 2 of 4
with (sample_df, plot(Timestamp, Voltage, ylab="Voltage", xlab="datetime", type="l") )
# plot 3 of 4
...
# plot 4 of 4
with (sample_df, plot(Timestamp, Global_active_power, ylab="Global_active_power",xlab="datetime", type="l") )
# save the file
dev.off()
#
if (DEBUGR == TRUE) {
